...
P_fa_detection_table = table({ ' ' }, [0] , [0],[0],'VariableNames',{'FileName' 'P_detection' 'P_fa' 'treshold' })  ;
...
P_fa_lst = [] ; 
	...
	for treshold = 0%:0.1:3
		...
		P_detection_vector = [P_detection_vector ; P_detection] ; 
		P_fa = integral( @(data)GG_h00(data,muu,alphaa , betta) , treshold , inf );
		P_fa_vector = [P_fa_vector ; P_fa] ;
		P_detection_vector 
		P_fa_vector
	end	
	P_detection_lst = [P_detection_lst ;P_detection_vector ] ; 
	P_fa_lst = [P_fa_lst ; P_fa_vector] ; 
	%figure
	plot (P_fa_vector' , P_detection_vector');
	title('ROC for all Images')
	xlabel('P FA') ; 
	ylabel('P detection')
	%treshold =  1 ; 
	%P_detection = 0.5 *integral( @(data)GG_h11(data,muu,alphaa , betta, 1, 0.2) , treshold , inf ) + ...
	%					   0.5 *integral( @(data)GG_h11(data,muu,alphaa , betta, -1, 0.2) , treshold , inf )	;
	%P_fa = integral( @(data)GG_h00(data,muu,alphaa , betta) , treshold , inf );
	...
	%P_fa_detection_table  = [P_fa_detection_table ; {baseFileName, P_detection , P_fa ,treshold}] ; 
...
Marker_Counter = 1;
%figure6=figure;
sz_Pdet = size(P_detection_lst) ; 
%Markers = {'+','o','*','x','v','d','^','s','>','<'};
%for iii = [1: sz_Pdet(1)]
%	%plot (P_fa_lst (iii,:) , P_detection_lst (iii,:), strcat('-',Markers{Marker_Counter}) );
%	plot (P_fa_lst (iii,:) , P_detection_lst (iii,:)) ; 
%	%Marker_Counter=Marker_Counter+1; 
%end
%P_detection_lst
%P_fa_lst 

display(P_fa_detection_table);
hold('off') ; 
